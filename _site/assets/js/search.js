(function () {
	...
	function displaySearchResults(results, query) {
		...
		if (results.length) {
			var resultsHTML = "";
			var searchVersions = ["Previous","all"]
			results.forEach(function (result) {
			
  				var item = window.data[result.ref]

				// Versioning is disabled
				if (("true" == "false") && (item.version != "all")) {
				    return
				}
				...
				if (("true" == "true") && ("true" == "true") && (!searchVersions.includes(item.version))) {
				    return
				}
					...
					if (searchVersions.length == 1 ||  "true" == "false"){
						versionBadge = ""
					...
					resultsHTML += "<li><h4><a href='/UAIbot.github.io" + item.url.trim() + "'>" + titlePreview + "</a></h4><p>" + versionBadge +"<small>" + contentPreview + "</small></p></li>";
				}
			});
